...
- **Unread badges** on both floating buttons: the parent's counts new kid
  ...
  reply you're mid-typing is never wiped.
- Parent-side reports are grouped by kid (accent-dot + name) with type chips.

### Database — NEW TABLES (manual step on deploy)
- Adds `feedback_threads` and `feedback_messages` (soft-deletes + timestamps).
- **Production is MySQL with no `artisan migrate`** — run **`deploy/feedback-tables.sql`**
  once in phpMyAdmin (Import). Local dev gets it via the normal migration.
